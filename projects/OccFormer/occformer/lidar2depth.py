...
class CreateDepthFromLiDAR(object):
    ...
    def project_points(self,
        ...
        if intrins.shape[-1] == 4:
            points = torch.cat(
                (points, torch.ones((points.shape[0], 1, 1, 1))), dim=2)
    ...
    def __call__(self, results):
        ...
        if self.dataset == 'kitti':
            ...
            lidar_filename = os.path.join(self.data_root,
                                          'data_velodyne/velodyne/sequences',
                                          seq_id,
                                          "velodyne",
                                          filename.replace(".png", ".bin"))
            lidar_points = np.fromfile(
                lidar_filename, dtype=np.float32).reshape(-1, 4)
        ...
        imgs, rots, trans, intrins, post_rots, post_trans, _, sensor2sensors = results[
            'img_inputs']
    ...
    def visualize(self, imgs, img_depths):
        ...
        for img_index in range(imgs.shape[0]):
            ...
            depth_points = torch.stack(
                (depth_points[:, 1], depth_points[:, 0], depth_i[depth_points[:, 0], depth_points[:, 1]]), dim=1)

            plt.figure(dpi=300)
            plt.imshow(img_i)
            plt.scatter(depth_points[:, 0], depth_points[:, 1],
                        s=1, c=depth_points[:, 2], alpha=0.2)
            plt.axis('off')
            plt.title('Image Depth')

            plt.savefig(os.path.join(
                out_path, 'demo_depth_{}.png'.format(img_index)))
            plt.close()
